<template>
  <div class="page">
    <m-header :title="coinTxt&&coinTxt['credit1_text']+'充值'" :canback="Boolean(1)"></m-header>
    <div class="body">
      <ul>
        <li class="list">
          <div class="name">剩余{{coinTxt&&coinTxt['credit1_text']}}</div>
        ...
        <li class="list-last">
          ...
          <div class="code remittance" v-if="formData.paytype==1||formData.paytype==2">
          ...
          <div class="code remittance" v-if="formData.paytype==1||formData.paytype==2">
            <span class="name inp-name">到账{{coinTxt&&coinTxt['credit1_text']}}</span>
            <div class="content">
              <input class="inp" type="number" v-model="hkmoney" readonly :placeholder="'到账'+coinTxt&&coinTxt['credit1_text']">
            </div>
          </div>
          <div class="code remittance" v-if="formData.paytype==1||formData.paytype==2">
          ...
          <div class="code remarks" v-if="formData.paytype==1||formData.paytype==2">
            <div class="name">备注码</div>
            <div class="content remarksCode">{{data?data.bzcode:''}}</div>
          </div>
        </li>
      </ul>
      <div class="sub" @click="recharge" v-if="formData.paytype==1||formData.paytype==2">提交</div>
...
export default {
  ...
  computed:{
    ...mapGetters(['api','uid','coinTxt'])
  },
  methods:{
    initData(){
      // this.payPicker.setData(this.payList)
      this.type = this.$route.query.type;
      console.log('text',this.coinTxt[this.type+'_text'])
...
// console.log(items)
          ...
          if(items[0].value == 1){
            ...
            _This.payCode = _This.api+_This.data.addr_img_url;
          }
      });
    },
    // 充值信息
    getInfo(id){
      ...
      Owner.getRechargeInfo({id}).then(res=>{
          if(res.code == 1){
              ...
              for(let item in res.data.data.hkfs){
                paywayArr.push({value:item,text:res.data.data.hkfs[item]})
              }
              this.payPicker.setData(paywayArr)
