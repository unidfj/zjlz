    ...
    <div class="page" >
        ...
        <section class="body">
            <ul class="order-info" v-show="temp">
                ...
                <li>预约/即抢采{{coinTxt&&coinTxt['credit1_text']}}机：{{parseInt(temp.money)}}/{{parseInt(temp.money2)}}</li>
                <li>智能合约收益：{{temp.days}}天/{{parseInt(temp.per)}}%</li>
                <li>可挖{{coinTxt&&coinTxt['credit2_text']}}：{{parseInt(temp.credit2)}}枚</li>
                <li>可挖{{coinTxt&&coinTxt['credit4_text']}}：收益{{parseInt(temp.credit4_per)}}%</li>
            ...
            <ul class="certify-info" v-show="(type=='buy' &&detailType != 1)||(type=='sell' && detailType >= 1)">
                ...
                <div class="btn-group" v-show="(type=='buy' && detailType == 0 && temp.status==0)||(type=='sell' && detailType == 1)||(type=='buy'&& temp.status == 1)||(type == 'sell'&& temp.status4 == 2)">
                    ...
                    <button v-show="(type == 'sell'&& (temp.status4 == 2||temp.status4 == 3))" class="btn btn-red" @click.stop="appeal">申诉</button>
                </div>
            </ul>
        </section>
...
export default {
    ...
    computed:{
        ...mapGetters(['temp','userInfo','uid','api','coinTxt'])
    },
    methods:{
        initData(){
        ...
        pay(pwd){
            ...
            let params = {};
            if(this.type =='buy'){
                params = Object.assign({},this.formData,{'password':pwd,'status':'1'})
            }else {
                params = Object.assign({},this.formData,{'password':pwd,'status':'7'})
                delete params.image;
            }
...
@import '~link-less';
    .body {
        ...
        .certify-info {
            ...
            .file {
                position: absolute;left: @margin-primary;top: 0;
                width: 352px;height: 210px;
                opacity: 0;
            }
            .btn-group {
