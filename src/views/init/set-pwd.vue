<template>
    <div class="page">
        ...
        <section class="body">
            <form class="tj-form">
                <div class="form-item">
                    <label for="name">{{type=='login'?'登录密码':'交易密码'}}</label>
                    <input v-if="type=='login'" type="password" placeholder="6-15位（不能全是数字或字母）" v-model="formData.newpassword" >
                ...
                <div class="form-item">
                    ...
                    <input v-if="type=='login'" type="password" placeholder="6-15位（不能全是数字或字母）" v-model="formData.password">
                ...
                <button type="button" class="btn-submit btn-blue" @click="confirm">确定</button>
            </form>
...
// return ;
            // 发送验证码
            Base.sendCode({mobile:this.userInfo.mobile, event: "register"}).then((res)=>{
                mui.toast(res.msg);
            })            
        },
        ...
        confirm(){
            ...
            if(this.type == 'login'){
                ...
                Init.changePwd(this.formData).then(res=>{
                    if(res.code == 1){
                        setTimeout(() => {
                            Load.loadEnd(this);
                            mui.toast(res.msg);
